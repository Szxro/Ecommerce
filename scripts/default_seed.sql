...
BEGIN
	...
	VALUES
		...
		('Username','Represent the username of the user',@STRING_ID,GETUTCDATE(),GETUTCDATE(),0,CONVERT(datetime2,'1753-1-1'));
...
BEGIN
	...
	SET @TEMPLATE_ID = (SELECT TOP 1 A.id FROM template A INNER JOIN template_category B ON A.template_category_id = B.id WHERE B.[name] = 'welcome_message');
